...
class EEGwithNoise(object):
    def __init__(self, EEG_data, NOS_data, batch_size=128):
        ...
        for value in 10 ** (0.05 * np.linspace(-7.0, 2.0, num=11)):
    ...
    def shuffle(self):
        ...
        self.SNR_value = 10 ** (np.random.uniform(-7, 2, (self.EEG_data.shape[0])) * 0.05)
```

코드가 좀 긴데 다 볼 필요는 없고, 우선 EMG와 EOG는 BG-Attention과 같은 방식으로 노이즈 데이터를 생성한다. <br>
하지만 여기엔 hybrid data가 존재하는데, 최솟값을 기준으로 합쳐서 noise data를 만든다. <br>
그리고 EEG와 noise를 이전의 lambda와 함께 섞어줌으로써 데이터를 생성한다.

조금 다른점은 코드인데 여기선 min function을 사용해서 데이터를 잘라냈다. <br>
확실히 BG 논문보다 코딩을 잘한다는 게 느껴진다.

나도 이렇게 대충 정리하면 안 될 거 같긴 한데, 언제 봐도 쉽게 구별 가능할 정도라 상관없을 것 같다.
